...
let
	...
	s1 = scatter!(ax1, nearby_stations.longitude, nearby_stations.latitude,
		color = nearby_stations.mk_S, colormap = :RdBu, markersize = 18, label = "Mann-Kendall S")
	Colorbar(fig[1, 2], s1, label = "S Statistic")

	# Plot my station with a star (on top) using the same colormap
	my_mk_S = nearby_stations[nearby_stations.stnid.==my_stnid, :mk_S][1]
	scatter!(ax1, [my_station.longitude], [my_station.latitude],
		color = my_mk_S, colormap = :RdBu, markersize = 48, marker = :star5, strokewidth = 2, strokecolor = :black, label = "$(my_station.name)")
